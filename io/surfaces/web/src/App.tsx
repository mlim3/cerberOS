...
function App() {
  ...
  const selectedConversationSidebarPreview =
    !selectedTaskId ? '' : tasks.find(t => t.id === selectedTaskId)?.lastUpdate ?? ''
  ...
  }, [selectedTaskId, selectedConversationSidebarPreview, streamingForTaskId])
    ...
    const id = setInterval(() => {
      ...
      for (const task of working) {
        ...
        if (nextAt === undefined || nextAt === 0 || now >= nextAt) {
          const intervalMs = Math.floor(Math.random() * 21) * 100 + 2000
          taskNextHeartbeatAtRef.current[task.id] = now + intervalMs
  ...
  }, [selectedTaskId, refetchConversations])
