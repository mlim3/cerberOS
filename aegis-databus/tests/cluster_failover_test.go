...
func TestFRDB007_ClusterFailover(t *testing.T) {
	...
	// Phase 2: kill nats-1, measure reconnect time (FR-DB-007: < 5s)
	cmd := exec.Command("docker", "stop", "nats-1")
	t0 := time.Now()
	if err := cmd.Run(); err != nil {
		t.Logf("docker stop nats-1: %v (may need docker permissions)", err)
		t.Skip("cannot run docker stop; run manually: docker stop nats-1")
	}
	defer exec.Command("docker", "start", "nats-1").Run()
