...
CREATE INDEX idx_conversations_updated_at ON chat_schema.conversations(updated_at DESC);

CREATE TABLE chat_schema.tasks (
    id UUID PRIMARY KEY,
...
CREATE INDEX idx_chat_tasks_conversation_id ON chat_schema.tasks(conversation_id, updated_at DESC);
CREATE INDEX idx_chat_tasks_user_id ON chat_schema.tasks(user_id, updated_at DESC);
CREATE INDEX idx_chat_tasks_orchestrator_task_ref ON chat_schema.tasks(orchestrator_task_ref);
...
CREATE UNIQUE INDEX idx_messages_conversation_idempotency
...
CREATE TABLE personal_info_schema.user_facts_archive (
...
CREATE INDEX idx_user_facts_archive_user_id ON personal_info_schema.user_facts_archive(user_id);
CREATE INDEX idx_user_facts_archive_fact_id ON personal_info_schema.user_facts_archive(fact_id);
CREATE INDEX idx_user_facts_archive_reason ON personal_info_schema.user_facts_archive(archive_reason);
...
-- orchestrator_schema
-- ==========================================
CREATE TABLE orchestrator_schema.orchestrator_records (
    id UUID PRIMARY KEY,
...
-- scheduler_schema
-- ==========================================
CREATE TABLE scheduler_schema.scheduled_jobs (
    id UUID PRIMARY KEY,
...
CREATE TABLE scheduler_schema.scheduled_job_runs (
    id UUID PRIMARY KEY,
...
CREATE INDEX idx_scheduled_jobs_status_next_run_at ON scheduler_schema.scheduled_jobs(status, next_run_at);
CREATE INDEX idx_scheduled_job_runs_job_id_started_at ON scheduler_schema.scheduled_job_runs(job_id, started_at DESC);
