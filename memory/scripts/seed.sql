...
-- 2. chat_schema.conversations
INSERT INTO chat_schema.conversations (id, user_id, title, created_at, updated_at)
VALUES
    ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '11111111-1111-1111-1111-111111111111', 'Hi, can you help me write a Python script for data analysis?', NOW() - INTERVAL '10 minutes', NOW() - INTERVAL '8 minutes')
ON CONFLICT (id) DO NOTHING;

-- 3. chat_schema.messages (Conversation A for user 1)
INSERT INTO chat_schema.messages (id, conversation_id, user_id, role, content, token_count, idempotency_key, created_at)
VALUES 
...
-- 4. personal_info_schema.user_facts (for user 1 and 2)
...
-- 5. vault_schema.secrets (for user 1)
