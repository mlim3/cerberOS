...
func TestParallel_ResultsInOriginalOrder(t *testing.T) {
	...
	result, err := RunLoop(ctx, log, spawnCtx, nil, nil, nil, option.WithBaseURL(srv.URL))
	if err != nil {
...
func TestParallel_ConcurrentExecutionFasterThanSequential(t *testing.T) {
	...
	actStart := time.Now()
	result, err := RunLoop(ctx, log, spawnCtx, nil, nil, nil, option.WithBaseURL(apiSrv.URL))
...
func TestParallel_SteeringInterruptCancelsAllConcurrentCalls(t *testing.T) {
	...
	start := time.Now()
	_, err := RunLoop(ctx, log, spawnCtx, nil, steerer, nil, option.WithBaseURL(apiSrv.URL))
