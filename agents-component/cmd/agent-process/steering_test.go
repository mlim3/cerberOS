...
func TestLoop_CancelDirectiveTerminatesTask(t *testing.T) {
	...
	_, err := RunLoop(ctx, log, spawnCtx, nil, steerer, nil, option.WithBaseURL(srv.URL))
	if err == nil {
...
func TestLoop_RedirectDirectiveInjectedIntoHistory(t *testing.T) {
	...
	result, err := RunLoop(ctx, log, spawnCtx, nil, steerer, nil, option.WithBaseURL(srv.URL))
	if err != nil {
